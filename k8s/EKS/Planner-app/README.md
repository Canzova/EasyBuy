...
flowchart TD
    ...
    subgraph EKS["Amazon EKS Cluster (VPC: ap-south-1)"]
        subgraph KubeSystem["Namespace: kube-system"]
            LBC[AWS Load Balancer Controller]
            CSI[AWS EBS CSI Driver Controller]
        end
        
        subgraph Namespace["Namespace: planner-app"]
            Ingress[Kubernetes Ingress: planner-ingress]
            SpringBootSvc[Service: planner-service\nType: ClusterIP:8080]
            
            subgraph SpringBootPods["Spring Boot Workload (Replicas: 2)"]
                AppPod1["Pod: planner-deployment-1\nContainer: canzova/planner:1.0"]
                AppPod2["Pod: planner-deployment-2\nContainer: canzova/planner:1.0"]
            end
            
            MySQLSvc[Headless Service: mysql-service\nType: ClusterIP:None:3306]
            
            subgraph MySQLPod["MySQL Database Workload (Replicas: 1)"]
                DBPod["StatefulSet Pod: mysql-0\nContainer: mysql:9\nMount: /var/lib/mysql"]
            end
            
            PVC["Dynamic PVC: mysql-persistent-storage-mysql-0\nStorageClass: ebs-gp3-sc"]
            Configs[ConfigMap: planner-config-map\nSecret: planner-secrets]
        end
    end
    
    subgraph AWSStorage["AWS Cloud Storage"]
        EBS[(AWS EBS Volume: gp3\nSingle Availability Zone)]
    end
    ...
    AppPod1 -->|Connects to DB_HOST: mysql-service:3306| MySQLSvc
    AppPod2 -->|Connects to DB_HOST: mysql-service:3306| MySQLSvc
    MySQLSvc --> DBPod
    
    Configs -.->|Env Vars| SpringBootPods
    Configs -.->|Env Vars| MySQLPod
    
    DBPod -->|Volume Mount| PVC
    CSI -.->|Creates & Attaches gp3 EBS| EBS
    EBS --> DBPod

    %% Subgraphs transparent styling to prevent theme clashing
    style EKS fill:none,stroke:#38BDF8,stroke-width:2px,stroke-dasharray: 5 5
    style KubeSystem fill:none,stroke:#F59E0B,stroke-width:2px,stroke-dasharray: 4 4
    style Namespace fill:none,stroke:#38BDF8,stroke-width:2px,stroke-dasharray: 4 4
    style AWSStorage fill:none,stroke:#22C55E,stroke-width:2px,stroke-dasharray: 4 4
    style SpringBootPods fill:none,stroke:#0284C7,stroke-width:1px
    style MySQLPod fill:none,stroke:#0284C7,stroke-width:1px

    %% Explicit high-contrast nodes
    ...
    style Ingress fill:#0369A1,color:#FFFFFF,stroke:#38BDF8,stroke-width:2px
    style SpringBootSvc fill:#1E293B,color:#FFFFFF,stroke:#94A3B8,stroke-width:2px
    ...
    style PVC fill:#1E293B,color:#FFFFFF,stroke:#38BDF8,stroke-width:2px
    style Configs fill:#1E293B,color:#FFFFFF,stroke:#38BDF8,stroke-width:2px
    style EBS fill:#15803D,color:#FFFFFF,stroke:#4ADE80,stroke-width:2px
```
